...
Rain probability: 0%"></textarea>
    ...
    if (currentForecastData) {
        displayForecastResult(currentForecastData, currentForecastTrackName);
    }
...
function parseWeatherFromText(text) {
    ...
    if (!text) return { weather, trackName };
...
function parseForecastText(textInput) {
    ...
    displayForecastResult(result.weather, result.trackName);
}

function displayForecastResult(w, trackName) {
    currentForecastData = w;
    currentForecastTrackName = trackName;
...
function renderWeatherSection(container, w, trackName, dataVarName = 'cachedNextRaceData') {
    const card = document.createElement('div');
    card.className = 'card';
    card.style.gridColumn = '1 / -1';
    ...
    let html = `<div class="card-header"><h3>Weather Forecast${trackName ? ' - ' + trackName : ''}</h3></div><div style="padding:15px;">`;

    // Current Weather Table
    if (w.q1Temp !== undefined || w.q2Temp !== undefined) {
        html += `
            <h4 style="border-bottom:1px solid var(--border); padding-bottom:5px; margin-bottom:10px;">Current Weather</h4>
            <table class="setup-table" style="width:100%; max-width:600px; margin-bottom:20px;">
                <thead>
                    <tr><th>Session</th><th>Weather</th><th>Temp</th><th>Humidity</th></tr>
                </thead>
                <tbody>
                    <tr>
                        <td style="font-weight:bold;">Practice / Qualify 1</td>
    ...
    if (w.raceQ1TempLow !== undefined) {
        html += `
            <h4 style="border-bottom:1px solid var(--border); padding-bottom:5px; margin-bottom:10px;">Race Forecast</h4>
            <div style="overflow-x:auto;">
            <table class="setup-table" style="width:100%; max-width:800px;">
                <thead>
                    <tr><th>Metric</th><th>Start - 30m</th><th>30m - 1h</th><th>1h - 1h30m</th><th>1h30m - 2h</th></tr>
                </thead>
                <tbody>
        `;
        // Temp
        html += `<tr><td style="font-weight:bold;">Temp</td>`;
        for(let i=1; i<=4; i++) html += `<td>${w[`raceQ${i}TempLow`] !== undefined ? w[`raceQ${i}TempLow`] + '° - ' + w[`raceQ${i}TempHigh`] + '°' : '-'}</td>`;
        html += `</tr>`;
        // Humidity
        html += `<tr><td style="font-weight:bold;">Humidity</td>`;
        for(let i=1; i<=4; i++) html += `<td>${w[`raceQ${i}HumLow`] !== undefined ? w[`raceQ${i}HumLow`] + '% - ' + w[`raceQ${i}HumHigh`] + '%' : '-'}</td>`;
        html += `</tr>`;
        // Rain
        html += `<tr><td style="font-weight:bold;">Rain Probability</td>`;
        for(let i=1; i<=4; i++) html += `<td>${w[`raceQ${i}RainPLow`] !== undefined ? w[`raceQ${i}RainPLow`] + '% - ' + w[`raceQ${i}RainPHigh`] + '%' : '-'}</td>`;
        html += `</tr>`;

        html += `</tbody></table></div>`;
        html += `<div style="margin-top:15px;"><button onclick="searchForSimilarWeather(${dataVarName}.weather, 'rain')" style="padding:6px 12px; cursor:pointer; background:var(--accent); color:white; border:none; border-radius:4px;">Find Races with Similar Rain</button></div>`;
    }
